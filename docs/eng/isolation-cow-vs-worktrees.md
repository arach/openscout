...
>
> **Current status (2026-07-01):** only the shared `createAgentWorkspace`
> *worktree* helper shipped — a pure de-duplication of the two inline
> `createGitWorktree` bodies; `git worktree` remains the sole strategy. The COW
> clone, workspace teardown, and merge-back are all **deferred**: a prototype was
> adversarially reviewed (Codex) and several correctness/safety blockers were
> reproduced. See "Part 5 → Review blockers" before reviving COW.
...
> **Status:** design + deferred plan. What actually shipped is only the shared
> *worktree* helper (a de-duplication); the COW clone, teardown, and merge-back
> are all deferred. A COW+teardown prototype was adversarially reviewed and
> several blockers were reproduced (see "Review blockers" below) — those must be
> resolved before COW is revived. Merge-back is the hardest, destructive-if-wrong
> part and is planned here for when the COW workspace lands.

### What shipped

- **`createAgentWorkspace(projectRoot, agentName, requestedBranch)`**
  (`packages/runtime/src/agent-workspace.ts`) — the single shared helper that
  replaced the two byte-identical inline `createGitWorktree` bodies. It
  materializes a **`git worktree`** on branch `scout/<agentName>` under
  `<projectRoot>/.scout-worktrees/<agentName>`, reuses an existing one, and
  returns `{ path, branch }`. `git worktree` is the *only* strategy — a pure
  de-duplication with no behavior change. Both mobile `createGitWorktree` shims
  delegate here.

### What was prototyped then deferred

A COW clone (`cp -c -R` APFS clonefile) + terminal-flight teardown were built and
reviewed, then pulled back out. They are **not** in the shipped helper.

### Review blockers (why COW + teardown are deferred)

An adversarial review reproduced these against the prototype:

1. **`cp -c -R <root> <root>/.scout-worktrees/<new>` copies sibling workspaces.**
   macOS `cp` does not skip `.scout-worktrees` as a directory, so clones nest
   other agents' workspaces (cross-agent exposure, unbounded clone growth). Fix:
   exclude `.scout-worktrees`, or clone to a location outside the copied source.
2. **`cp -c` silently degrades to a full copy** when clonefile is unsupported
   (`man cp`), so the scratch probe can "pass" by real-copying and production can
   full-copy a large repo on a non-reflink volume. Fix: a force-clone probe/API
   that fails rather than degrades (native `clonefile`/`fclonefileat`, or
   `COPYFILE_FICLONE_FORCE` for the file probe).
3. **COW-cloning a gitfile worktree mutates the operator's checkout.** If
   `projectRoot/.git` is a gitfile, the clone's `git checkout -b` changed the
   *source* worktree's branch. Fix: only COW when `.git` is a real directory, or
   repair an independent gitdir before any git command.
4. **Teardown is lifetime-unsafe.** Firing by `targetAgentId` on any terminal
   flight can delete the cwd of a persistent/reused agent that still has an
   active flight, and leaves the relay override pointing at a deleted path. Fix:
   tie workspace ownership to a specific one-time flight/session and refcount
   active flights before teardown.
5. **The exported teardown guardrail is string-based** (`includes`/`startsWith`),
   so `<project>/.scout-worktrees/..` passes but resolves to the project root.
   Fix: `realpath` + `path.relative` non-`..` child check; reject the
   `.scout-worktrees/` root itself.
6. **Concurrent create race:** the loser's `cp` fails (dest exists) and the
   caller `rm`s the winner's workspace. Fix: per-workspace lock, or clone to a
   unique temp dir then atomic-rename; never clean up a path you did not create.
7. Reuse is too trusting (any existing path), and COW vs worktree expose
   different source state (dirty tree vs clean HEAD) — behavior differs by
   filesystem.
8. **Teardown before merge-back destroys agent edits** — since merge-back is
   plan-only, auto-teardown discards filesystem changes not already captured in
   output. Teardown must not precede a merge-back/capture step.

The merge-back plan below assumes a *fixed* COW workspace (blockers 1–3, 6–7
resolved) and a *flight/session-scoped, capture-before-teardown* lifecycle
(blockers 4, 8). The `kind` discriminator it references is the future seam: a
**worktree** is already a git branch in the parent's object DB (native merge),
whereas a **clone** is a standalone repo whose objects must be transferred first.
