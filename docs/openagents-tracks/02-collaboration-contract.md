...
This aligns with the current broker-first direction in [OpenScout Architecture](/Users/arach/dev/openscout/docs/ARCHITECTURE.md), the durable runtime model in [Relay Architecture](/Users/arach/dev/openscout/packages/relay/docs/architecture.md), and the `question` / `work_item` split in [Collaboration Workflows V1](/Users/arach/dev/openscout/docs/collaboration-workflows-v1.md).
