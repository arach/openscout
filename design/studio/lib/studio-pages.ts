...
export const STUDIO_PAGES: StudioPage[] = [
  ...
  {
    href: "/studies/project-picker",
    label: "Project Picker · Search First",
    bucket: "studies",
    surface: "macos",
    family: "comms",
    status: "concept",
    source: [
      "design/studio/views/project-picker.tsx",
      "design/studio/views/project-picker.module.css",
      "apps/macos/Sources/Scout/ScoutSessionService.swift",
    ],
    blurb:
      "A reusable ProjectSearch core shown in two presentations: an anchored New Conversation picker and a global finder. Free typing matches name · path · branch; zero-query suggestions are recent-first; paths keep repeated repos and worktrees unambiguous.",
  },
  {
  ...
  {
    href: "/studies/operator-console-themes",
    label: "Operator Console · six themes",
    bucket: "studies",
    surface: "cross",
    status: "concept",
    source: [
      "design/studio/views/operator-console-themes.tsx",
      "docs/design/operator-console-themes.md",
      "docs/design/operator-console-harvest.md",
    ],
    blurb:
      "The six themes from the Operator Console mock (Mission · Atelier · Unit 47 · Meridian · Porcelain · Editor), extracted and decomposed onto six axes instead of copied as welded blocks — the defect appearance-decomposed-picker.md §0.1 named in our own picker. Canvas, accent, and radius are already in our contract; type, texture, and light are new, and they do not cost the same. The frame runs a real multi-agent thread (operator → grok → kimi, ending in a live working turn) so all four turn registers get exercised under every preset. Editor is the no-texture control.",
  },
  {
    href: "/studies/turn-instruments",
    label: "Turn · live instrument",
    bucket: "studies",
    surface: "web",
    status: "concept",
    source: [
      "design/studio/views/turn-instruments.tsx",
      "docs/design/operator-console-harvest.md",
      "packages/web/client/screens/chat/ConversationPanels.tsx",
    ],
    blurb:
      "The agent turn as a live instrument (harvest §2, §3.2/§3.4): a four-column steps ledger with a tabular Took column shown next to the shipped WorkingTurnSteps, an 11-bar activity sparkline sized for a turn header, a flat elapsed hero numeral (no box), and the keycap chip for the shipped chords. One accent; working is motion; failure alone is danger.",
  },
  {
  ...
  },
  {
    ...
    status: "concept",
    source: [
      "design/studio/views/scout-ios-notifications.tsx",
      "apps/ios/Scout/HomeSurface.swift",
      "apps/ios/Scout/RootView.swift",
      "apps/ios/Scout/AppModel.swift",
      "packages/runtime/src/mobile-push.ts",
    ],
    blurb: "A reduced Home with one attention gateway, and an opaque-push notification detail that supports read, dismiss, and explicit navigation.",
  },
  {
    href: "/studies/scout-ios-notification-detail",
    label: "Scout iOS · Notification detail",
    bucket: "studies",
    surface: "ios",
    family: "scout-ios",
    status: "draft",
    source: [
      "design/studio/views/scout-ios-notification-detail.tsx",
      "design/studio/components/scout-ios/notification-surfaces.tsx",
      "docs/design/ios-home-simplification-and-notification-detail.md",
      "apps/ios/Scout/HomeSurface.swift",
      "apps/ios/Scout/RootView.swift",
    ],
    blurb: "The implementation-ready lab for the same direction as Scout iOS · Notifications, built on the shared components/scout-ios kit: Home cut to three fixed zones (Needs you · one flat Activity log · Ask), and the notification as a PUSHED page with eight authored states — resolving, resolved, acted, dismissed, handled-elsewhere, offline, send-failed, plus the OS banner. Act · Mark read · Dismiss are three separate verbs and only Open ↗ navigates; Dismiss says out loud that the agent keeps waiting. Contracts, touchpoints and delivery slices in the linked spec.",
  },
  {
  ...
  {
    href: "/studies/ios-home-vanilla",
    label: "Scout iOS · Everyday Home",
    bucket: "studies",
    surface: "ios",
    family: "scout-ios",
    status: "concept",
    source: [
      "design/studio/views/ios-home-vanilla.tsx",
      "design/studio/views/crown-complications.tsx",
      "apps/ios/Scout/RootView.swift",
      "apps/ios/Scout/HomeSurface.swift",
    ],
    blurb: "Persona-lane concept: the 'normal person' lane for Scout iOS — light grouped canvas, rounded cards, one restrained accent, calm status pills where the Operator lane has hex complications. V2: the lane launches composer-first (the 'smart, beautiful default') — one-line Ask-the-fleet composer that expands on tap, dominant host as ambient context, most-recent harness/model/effort chips + project pre-filled, no tab bar; the v1 dashboard becomes Overview (supervision), reached via the top-left anchor. Lane switcher Everyday vs Operator (Operator is a reference thumbnail only); Landing: Composer·Overview preference; ?lane / ?landing / ?composer=open params for deterministic captures.",
  },
  {
    href: "/studies/ios-concierge",
    label: "Scout iOS · The Concierge",
    bucket: "studies",
    surface: "ios",
    family: "scout-ios",
    status: "concept",
    source: [
      "design/studio/views/ios-concierge.tsx",
      "design/studio/views/ios-home-vanilla.tsx",
    ],
    blurb: "Conversational meta-interface over the app's own functions and state — not an agent orchestrator. Reads are free (fetch, status, summarize), mutations are gated as chat artifacts (amber approval card, confirm-before-send draft). Entry as a sheet over the Launch frame with voice-first Talk badge and suggestion chips. Privacy ladder: on-device (later) → mesh-local → BYOC (Cloudflare/AWS/e2b/exe.dev) → BYOK (default); the tool surface is the durable asset, the model is swappable. ?frame=chat|entry|ladder for deterministic captures.",
  },
  {
    href: "/studies/ios-launch-craft",
    label: "Scout iOS · Launch Craft",
    bucket: "studies",
    surface: "ios",
    family: "scout-ios",
    status: "concept",
    source: [
      "design/studio/views/ios-launch-craft.tsx",
      "apps/ios/Scout/ComposerKit.swift",
      "apps/ios/Scout/ModelPickerPopover.swift",
      "apps/ios/Scout/NewSessionSurface.swift",
    ],
    blurb: "Keynote-quality craft reference for the composer-first Launch frame (Opus 5 pass). The frame is a 'dispatch slip': warm-paper canvas, writing sheet docked to the keyboard, keyboard as the call to action. Three config treatments switchable via ?treatment=1|2|3 — 01 Sentence (config as prose on the canvas, values tappable, owner's favored direction), 02 Masthead (place = project·host as masthead above the sheet, engine = harness·model·effort as the model token in the composer action row), 03 Manifest (whole route compressed to one strip, full decision set one disclosure away). Composer rebuilt to ComposerKit.swift order (+ attach · model token · mic immediately left of send), live send state, family/effort token grammar, SF type with two ranges only, filled-never-stroked marks. Includes the one-tap ModelPickerPopover port (harness logos → family → effort).",
  },
  {
    href: "/studies/composer-atlas",
    label: "Scout · Composer Atlas",
    bucket: "studies",
    surface: "ios",
    family: "message-composer",
    status: "concept",
    source: [
      "design/studio/views/composer-atlas.tsx",
      "design/studio/components/MessageComposer.tsx",
      "apps/ios/Scout/ComposerKit.swift",
      "apps/ios/Scout/NewSessionSurface.swift",
      "packages/web/client/components/MessageComposer/MessageComposer.tsx",
    ...
    blurb: "The consolidated reference for every message-input composer in the project — the view that did not exist. Canonical grammar first (TO / BODY / BAR; ComposerKit action row '+ attach · spacer · model token · mic · send'; outlined capsule token, accent-filled send dimmed until submittable), then 20 specimen cards across iOS studio treatments (launch-craft ×3, home-vanilla, concierge, SurfaceLab, steer), macOS (.composerBox idiom, reply/thread variants, session sheet, HUD runner), and web (the live MessageComposer atom + its production twin + ConversationComposer wrapper), plus special contexts (fleet-deck placement, steering-loop dispatch, PR assign, rail actions, channels). A 22-row matrix compares config placement / attach / model token / mic / send style across all of them, and six observations name the drift: config placement is the real open axis, two send materials ship, and filled-gray-chip tokens have drifted from the outlined reference.",
  },
  {
  ...
  },
  {
    ...
    bucket: "studies",
    surface: "web",
    family: "work-observability",
    status: "draft",
    source: [
      "design/studio/views/work-observability.tsx",
      "packages/web/client/screens/chat/ConversationScreen.tsx",
      "packages/web/client/screens/chat/ConversationHeader.tsx",
      "packages/web/client/screens/chat/conversation-model.ts",
      "packages/protocol/src/invocations.ts",
      "packages/protocol/src/collaboration.ts",
      "packages/protocol/src/harness-feature-support.ts",
    ],
    blurb:
      "A simpler work rail for live conversations: identity, one useful status sentence, and three concepts — Observe, Guide, Details. The study keeps broker provenance and honest Steer/Cue delivery semantics, but moves session handles, evidence, secondary routes, and destructive controls behind disclosure so the conversation remains primary.",
  },
  {
