    ...
    <main className="mx-auto max-w-page px-7 py-8">
      ...
      <Section title="Spacing scale" hint="Every step the studio actually uses">
        <div className="rounded-md border border-studio-edge bg-studio-surface p-5">
          <div className="mb-3 grid grid-cols-[60px_64px_1fr_minmax(220px,1fr)] gap-4 border-b border-studio-edge pb-2 font-mono text-2xs font-semibold uppercase tracking-eyebrow text-studio-ink-faint">
          ...
          <div className="[&>*+*]:border-t [&>*+*]:border-studio-edge">
            {SCALE.map((s) => (
              <div key={s.px} className="grid grid-cols-[60px_64px_1fr_minmax(220px,1fr)] items-center gap-4 py-2.5">
                <code className="font-mono text-sm tabular-nums text-studio-ink">{s.px}px</code>
                <code className="font-mono text-sm text-studio-ink-muted">p-{s.tw} · gap-{s.tw}</code>
                <div className="flex items-center gap-3">
                  <div className="h-3 rounded-[2px] bg-studio-canvas-alt ring-1 ring-studio-edge" style={{ width: `${s.px}px` }} />
                  <span className="font-mono text-2xs text-studio-ink-faint">{s.px}px</span>
                </div>
                <div className="font-sans text-md text-studio-ink-faint">{s.usage}</div>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-baseline justify-between">
        <div>
          <div className="font-sans text-lg font-medium text-studio-ink">{label}</div>
          <code className="font-mono text-xs text-studio-ink-faint">{sub}</code>
        </div>
        <div className="font-mono text-2xs uppercase tracking-eyebrow text-studio-ink-faint">{hint}</div>
      </div>
      <dl className="flex flex-wrap gap-x-3 gap-y-0.5 border-t border-studio-edge pt-1.5 font-mono text-2xs">
        {callouts.map(([k, v]) => (
...
function Avatar({ name, hue, size }: { name: string; hue: number; size: "sm" | "md" }) {
  const cls = size === "sm" ? "h-5 w-5 text-xs" : "h-6 w-6 text-xs";
  return (
  ...
  return (
    <div className="flex items-center gap-2.5 rounded-sm px-2 py-1.5 transition-colors hover:bg-studio-canvas-alt">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <span className="truncate font-sans text-md text-studio-ink">{a.name}</span>
        <span className="truncate font-mono text-xs" style={{ color }}>
          {a.state} <span className="text-studio-ink-faint">· {a.task}</span>
        </span>
      </div>
      <span className="shrink-0 font-mono text-2xs tabular-nums text-studio-ink-faint">{a.updatedAgo}</span>
    </div>
  );
}
...
function CompactRow({ a }: { a: Agent }) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-sm px-2 py-1 transition-colors hover:bg-studio-canvas-alt">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <span className="truncate font-sans text-md text-studio-ink">{a.name}</span>
        <span className="truncate font-mono text-2xs" style={{ color }}>{a.state}</span>
      </div>
      <span className="shrink-0 font-mono text-2xs tabular-nums text-studio-ink-faint">{a.updatedAgo}</span>
    </div>
  );
...
function ManifestRow({ a }: { a: Agent }) {
  ...
  return (
    <div className="flex items-baseline gap-4 border-b border-studio-edge px-3 py-1.5 last:border-b-0 transition-colors hover:bg-studio-canvas-alt">
      ...
      <span className="w-[64px] shrink-0 truncate font-sans text-md font-medium text-studio-ink">{a.name}</span>
      <span className="w-[88px] shrink-0 truncate font-mono text-2xs uppercase tracking-eyebrow" style={{ color }}>{a.state}</span>
      <span className="min-w-0 flex-1 truncate font-sans text-md text-studio-ink-faint">{a.task}</span>
      <span className="shrink-0 font-mono text-xs tabular-nums text-studio-ink-faint">{a.updatedAgo}</span>
    </div>
  );
}
