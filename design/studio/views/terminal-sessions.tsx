  ...
  return (
    <main className="mx-auto max-w-page px-7 py-8 text-studio-ink">
      <header className="mb-5 max-w-3xl">
        ...
        <h1 className="mt-1 font-display text-6xl font-medium leading-none text-studio-ink">
          Terminal sessions
        </h1>
        <p className="mt-3 max-w-3xl font-sans text-lg leading-relaxed text-studio-ink-faint">
...
function WindowTitlebar() {
  return (
    <div className="flex h-[30px] items-center gap-2 border-b border-studio-edge bg-studio-canvas-alt px-3">
      ...
      <span className="ml-2 font-mono text-2xs uppercase tracking-eyebrow text-studio-ink-faint">
        Scout sessions
      </span>
      <span className="ml-auto hidden font-mono text-2xs uppercase tracking-eyebrow text-studio-ink-faint md:inline">
...
function ModeStrip() {
  return (
    <div className="relative flex h-[34px] items-center justify-between border-b border-studio-edge bg-studio-surface px-3">
      ...
      <div className="absolute left-1/2 hidden -translate-x-1/2 items-center gap-2 font-mono text-2xs text-studio-ink-muted md:flex">
...
function WindowStatusbar() {
  return (
    <div className="flex h-[22px] items-center gap-2 border-t border-studio-edge bg-studio-canvas-alt px-3 font-mono text-2xs uppercase tracking-eyebrow text-studio-ink-faint">
  ...
  return (
    ...
    >
      ...
      <div className="flex items-baseline gap-2">
        ...
        <span className="truncate font-sans text-md font-semibold text-studio-ink">
          {session.label}
        </span>
        <HarnessBadge harness={session.harness} />
        <span className="ml-auto font-mono text-2xs uppercase tracking-eyebrow text-studio-ink-faint">
          {live ? "live" : session.surfaces.length === 0 ? "no surface" : "idle"}
        </span>
      </div>
      <div className="mt-1 truncate pl-[15px] font-mono text-2xs text-studio-ink-faint">
      ...
      <div className="flex flex-wrap items-center gap-3 border-b border-studio-edge px-4 py-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="truncate font-sans text-2xl font-semibold leading-none tracking-tight text-studio-ink">
              {session.label}
            </h2>
            <HarnessBadge harness={session.harness} />
          </div>
      ...
      <div className="min-h-0 flex-1 bg-studio-canvas p-4">
        <div className="flex h-full min-h-[392px] flex-col overflow-hidden rounded-[6px] border border-studio-edge-strong bg-[#070907] shadow-[0_12px_30px_-20px_rgba(0,0,0,0.85)]">
          <div className="flex h-8 items-center gap-2 border-b border-white/10 bg-black/25 px-3">
            ...
            <span className="ml-auto truncate font-mono text-2xs text-studio-ink-faint">
              {session.cwd}
            </span>
          </div>
          {activeSurface ? (
            <pre className="min-h-0 flex-1 overflow-hidden whitespace-pre-wrap px-4 py-3 font-mono text-md leading-6 text-[#c7e7cd]">
  ...
  return (
    <div className="flex items-center gap-2 border-b border-studio-edge bg-studio-surface px-4 py-2">
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        {session.surfaces.length === 0 ? (
          ...
          session.surfaces.map((surface) => (
            <button
              ...
              className={[
                "inline-flex items-center gap-1.5 rounded-[4px] border px-2 py-1 font-mono text-2xs font-semibold uppercase tracking-eyebrow transition-colors",
        ...
        <span className="ml-1 font-mono text-3xs uppercase tracking-eyebrow text-studio-ink-faint">
  ...
  return (
    <div className="mt-3 overflow-hidden rounded-[6px] border border-studio-edge bg-studio-canvas-alt">
      ...
      {observe ? (
        ...
        <div className="border-t border-studio-edge px-3 py-2 font-mono text-2xs text-studio-ink-faint">
          observe · {surface.backend === "tmux" ? "read-only relay mode" : "—"}
        </div>
      )}
      {surface.socketDir ? (
        <div className="border-t border-studio-edge px-3 py-1.5 font-mono text-3xs text-studio-ink-faint">
...
function CommandRow({ label, command }: { label: string; command: string }) {
  return (
    <div className="flex items-center gap-2 px-3 py-2">
      <span className="w-[52px] shrink-0 font-mono text-3xs font-semibold uppercase tracking-eyebrow text-studio-ink-faint">
        {label}
      </span>
      <code className="min-w-0 flex-1 truncate font-mono text-xs text-studio-ink-muted">
        {command}
      </code>
...
function CopyButton({ value }: { value: string }) {
  ...
  return (
    <button
      ...
      className="inline-flex h-6 shrink-0 items-center gap-1 rounded-[4px] border border-studio-edge bg-studio-canvas px-1.5 font-mono text-3xs font-semibold uppercase tracking-eyebrow text-studio-ink-faint transition-colors hover:border-studio-edge-strong hover:text-studio-ink"
    >
      <CopyIcon />
      {copied ? "copied" : "copy"}
    </button>
...
function NoSurfacePlaceholder() {
  return (
    <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-3 px-6 text-center">
      ...
      <p className="max-w-[320px] font-sans text-sm leading-relaxed text-studio-ink-muted">
...
function SurfaceRow({
  ...
  return (
    ...
    >
      ...
      <div className="mt-1 truncate font-mono text-2xs text-studio-ink-faint">
        {surface.sessionName}
        {surface.paneId ? ` · ${surface.paneId}` : ""}
      </div>
      {surface.note ? (
        <div className="mt-1 font-sans text-2xs leading-snug text-studio-ink-faint">
          {surface.note}
        </div>
      ) : null}
  ...
  return (
    <div className="rounded-[5px] border border-studio-edge bg-studio-canvas-alt px-2.5 py-2">
      <div className="flex items-center gap-2">
        ...
        {note ? (
          <span className="ml-auto font-mono text-3xs uppercase tracking-eyebrow text-studio-ink-faint">
            {note}
          </span>
        ) : null}
      </div>
      <div className="mt-1.5 grid grid-cols-[52px_1fr] gap-x-2 gap-y-0.5 font-mono text-2xs">
  ...
  return (
    <div className="border-t border-studio-edge bg-studio-canvas-alt px-3 py-3">
      <div className="grid gap-2 xl:grid-cols-[184px_1fr_1fr_auto_auto] xl:items-center">
        ...
        <button
          type="button"
          className="inline-flex h-9 items-center justify-center gap-1.5 rounded-[5px] border border-studio-edge-strong bg-studio-canvas px-4 font-mono text-2xs font-semibold uppercase tracking-eyebrow text-studio-ink-muted transition-colors hover:border-[color-mix(in_oklab,var(--scout-accent)_42%,transparent)] hover:text-scout-accent"
        >
          <MaterializeIcon />
          Materialize
        </button>
    ...
    <div className="grid h-9 grid-cols-2 overflow-hidden rounded-[5px] border border-studio-edge bg-studio-canvas">
      {(["tmux", "zellij"] as const).map((option) => (
        <button
          ...
          className={[
            "font-mono text-2xs font-semibold uppercase tracking-eyebrow transition-colors",
            backend === option
...
function DockField({ label, value }: { label: string; value: string }) {
  return (
    <div className="grid h-9 grid-cols-[64px_1fr] items-center rounded-[5px] border border-studio-edge bg-studio-canvas px-2">
      ...
      <span className="truncate text-right font-mono text-xs text-studio-ink">
        {value}
      </span>
    </div>
  );
...
function RailHeader({ title, count }: { title: string; count: number }) {
  return (
    <div className="flex items-center justify-between border-b border-studio-edge px-3 py-2.5">
      ...
      <span className="rounded-[2px] border border-studio-edge bg-studio-canvas px-1.5 py-px font-mono text-2xs tabular-nums text-studio-ink-muted">
        {count}
      </span>
    </div>
  );
...
function ActionButton({
  ...
  return (
    <button
      type="button"
      className={[
        "inline-flex h-8 items-center gap-1.5 rounded-[5px] border px-2.5 font-mono text-2xs font-semibold uppercase tracking-eyebrow transition-colors",
        accent
          ? "border-[color-mix(in_oklab,var(--scout-accent)_42%,transparent)] bg-scout-accent-soft text-scout-accent"
...
function SmallAction({ label, accent }: { label: string; accent?: boolean }) {
  return (
    <button
      type="button"
      className={[
        "h-7 rounded-[4px] border font-mono text-3xs font-semibold uppercase tracking-eyebrow transition-colors",
        accent
          ? "border-[color-mix(in_oklab,var(--scout-accent)_42%,transparent)] bg-scout-accent-soft text-scout-accent"
...
function HarnessBadge({ harness }: { harness: HarnessKind }) {
  return (
    <span className="rounded-[2px] border border-studio-edge bg-studio-canvas px-1.5 py-px font-mono text-3xs font-semibold uppercase tracking-eyebrow text-studio-ink-muted">
      {harness}
    </span>
  );
}
...
function SurfaceBadge({ surface }: { surface: TerminalSurface }) {
  return (
    <span className="inline-flex items-center gap-1 rounded-[2px] border border-studio-edge bg-studio-canvas px-1.5 py-px font-mono text-3xs font-semibold uppercase tracking-eyebrow text-studio-ink-muted">
      <span
...
function BackendLabel({ backend }: { backend: TerminalBackend }) {
  return (
    <span className="font-mono text-2xs font-semibold uppercase tracking-eyebrow text-studio-ink-muted">
      {backend}
    </span>
  );
}
...
function FactLine({ label, value }: { label: string; value: string }) {
  return (
    <div className="grid grid-cols-[58px_1fr] items-baseline gap-2 font-mono text-2xs">
