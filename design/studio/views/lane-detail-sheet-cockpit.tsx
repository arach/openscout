...
/**
 * Agent Lane · Detail Sheet — COCKPIT direction.
 *
 * The read-only fact dump reframed as a LIVE INSTRUMENT CLUSTER for one agent
 * working right now. The same data, but staged like a cockpit glance:
 *
 *   VITALS   — the live action as hero (streaming command well) + a prominent
 *              CONTEXT gauge framed as a token budget + a slim cadence meter.
 *              "alive · how full · how busy" answered in one look.
 *   CLUSTER  — runtime ids + raw token totals as a calm secondary readout.
 *   THE REST — ACTIONS → FILES → COMMANDS → PLANS → DOCS, unchanged in spirit.
 *
 * House rules held: flat surfaces (no cards around sections — structure from
 * rules / meters / dot-led rows), ONE emerald accent (no categorical color,
 * a near-full gauge never goes red), thin hairlines, no emoji.
 *
...
/* context budget — consistent with USAGE.ctxPct (42% of 258,400 ≈ 108,528) */
const CTX_BUDGET = 258_400;
const CTX_USED = Math.round((USAGE.ctxPct / 100) * CTX_BUDGET);
...
/* a small, restrained recent-activity series (events/turn) for the cadence meter */
const CADENCE = [3, 5, 4, 7, 6, 9, 8, 11, 7, 12, 10, 14, 9, 13];
...
/* ── VITALS ──────────────────────────────────────────────────────────────
   The cockpit hero: live action well + context budget gauge + cadence meter. */
function Vitals() {
  /* peak normalizes the cadence meter; clamp so the bars stay restrained */
  const peak = Math.max(...CADENCE);
  return (
    <section className="vitals">
      {/* NOW — the live action, foregrounded with a streaming live indicator */}
      <div className="now">
        <div className="now-top">
          ...
          <span className="now-label">executing now</span>
          <span className="now-turn">turn {AGENT.turn}</span>
        </div>
        <div className="now-action">
          ...
          <span className="now-caret" aria-hidden />
      ...
      {/* GAUGE — context as a token budget, the primary vital sign */}
      <div className="gauge">
        <div className="gauge-top">
          <span className="gauge-label">context</span>
          <span className="gauge-read">
            <b>{kfmt(CTX_USED)}</b>
            <span className="gauge-of"> / {kfmt(CTX_BUDGET)}</span>
            <span className="gauge-unit"> tokens</span>
            <span className="gauge-pct">{USAGE.ctxPct}%</span>
          </span>
        </div>
        <div className="gaugebar" role="meter" aria-label="context budget" aria-valuenow={USAGE.ctxPct} aria-valuemin={0} aria-valuemax={100}>
          <span className="gaugebar-fill" style={{ width: `${USAGE.ctxPct}%` }}>
            <span className="gaugebar-edge" aria-hidden />
          </span>
          {[25, 50, 75].map((t) => (
            <span key={t} className="gaugebar-tick" style={{ left: `${t}%` }} aria-hidden />
          ))}
        </div>
      </div>

      {/* CADENCE — a slim activity meter: how busy, right now */}
      <div className="cadence">
        <div className="cadence-meter" aria-hidden>
          {CADENCE.map((v, i) => {
            const h = 4 + Math.round((v / peak) * 14);
            const last = i === CADENCE.length - 1;
            return (
              <span key={i} className={`cad-bar${last ? " cad-bar--live" : ""}`} style={{ height: `${h}px` }} />
            );
          })}
        </div>
        <span className="cadence-read">
          <b>{STATS.events}</b> events · <b>{STATS.tools}</b> tools · {AGENT.age}
        </span>
      </div>
    </section>
  );
  ...
  return (
    <aside className="sheet">
      ...
      <div className="sheet-scroll">
        {/* VITALS — the cockpit hero: alive · how full · how busy */}
        <Vitals />
        ...
        {/* INSTRUMENT CLUSTER — runtime ids + raw token totals, calm + secondary */}
        <SecHead id="sec-runtime" label="Cluster" count="runtime · tokens" actions={<Ghost onClick={() => CopyCtx.copy(diagnostics, "diag")}>Copy diagnostics</Ghost>} />
        ...
        {/* raw token readout — the instrument cluster's secondary dials */}
        <div className="dials">
          {([
            ["in", fmt(USAGE.input)],
            ...
            <span className="dial" key={label}>
              <span className="dial-n">{v}</span>
...
export default function LaneDetailSheetCockpitPage() {
  ...
  return (
    <main className="lds-study">
      ...
      <header className="study-head">
        <div className="eyebrow">· studies · web · lane-detail-sheet · cockpit</div>
        <h1>Agent Lane · Detail Sheet — Cockpit</h1>
        <p className="sub">live cockpit / telemetry direction. lead with the executing command as the hero, read context as a token-budget gauge, sense cadence with a slim activity meter — runtime ids + raw totals settle below as a calm instrument cluster.</p>
      </header>
...
/* ── VITALS — the cockpit hero ── */
.vitals{padding:14px 14px 13px;border-bottom:1px solid ${C.edgeSoft};
  background:linear-gradient(180deg,color-mix(in srgb,${C.green} 4%,transparent),transparent 70%);
  display:flex;flex-direction:column;gap:13px}

/* NOW — live action well, foregrounded with a streaming indicator */
.now{display:flex;flex-direction:column;gap:7px}
.now-top{display:flex;align-items:center;gap:8px}
.live{position:relative;display:inline-flex;align-items:center;width:10px;height:10px;flex:none}
.live-dot{position:absolute;left:2px;top:2px;width:6px;height:6px;border-radius:50%;background:${C.green};
  box-shadow:0 0 0 0 color-mix(in srgb,${C.green} 50%,transparent);animation:livedot 1.9s ease-out infinite}
.live-stream{position:absolute;inset:0;border-radius:50%;border:1px solid color-mix(in srgb,${C.green} 40%,transparent);
  animation:livering 1.9s ease-out infinite}
@keyframes livedot{0%{box-shadow:0 0 0 0 color-mix(in srgb,${C.green} 45%,transparent)}70%,100%{box-shadow:0 0 0 5px transparent}}
@keyframes livering{0%{opacity:.7;transform:scale(.6)}100%{opacity:0;transform:scale(1.5)}}
.now-label{font-family:ui-monospace,monospace;font-size: var(--text-2xs);font-weight:700;text-transform:uppercase;letter-spacing:.16em;
  color:color-mix(in srgb,${C.green} 78%,${C.ink})}
.now-turn{margin-left:auto;font-family:ui-monospace,monospace;font-size: var(--text-2xs);color:${C.dim}}
.now-action{display:flex;align-items:flex-start;gap:8px;
  background:#070809;border:1px solid color-mix(in srgb,${C.green} 22%,transparent);border-radius:7px;
  padding:8px 9px;box-shadow:inset 0 1px 0 rgba(255,255,255,.03),0 0 0 3px color-mix(in srgb,${C.green} 6%,transparent),0 1px 2px rgba(0,0,0,.4)}
.now-prompt{flex:none;color:color-mix(in srgb,${C.green} 85%,${C.muted});font-weight:700;font-family:ui-monospace,monospace;font-size: var(--text-md);line-height:1.5}
...
.now-caret{flex:none;align-self:flex-end;width:7px;height:14px;margin:0 1px 1px 0;background:${C.green};border-radius:1px;
  animation:caret 1.05s steps(1) infinite;box-shadow:0 0 6px color-mix(in srgb,${C.green} 50%,transparent)}
@keyframes caret{0%,49%{opacity:.95}50%,100%{opacity:.12}}
.now-acts{display:inline-flex;align-items:center;gap:3px;flex:none}

/* GAUGE — context as a token budget, the primary vital */
.gauge{display:flex;flex-direction:column;gap:6px}
.gauge-top{display:flex;align-items:baseline;gap:8px}
.gauge-label{font-family:ui-monospace,monospace;font-size: var(--text-2xs);font-weight:700;text-transform:uppercase;letter-spacing:.16em;color:${C.muted}}
.gauge-read{margin-left:auto;font-family:ui-monospace,monospace;font-size: var(--text-sm);color:${C.dim};display:inline-flex;align-items:baseline;gap:0}
.gauge-read b{color:${C.ink};font-weight:600;font-size: var(--text-lg);font-variant-numeric:tabular-nums}
.gauge-of{color:${C.dim}}
.gauge-unit{color:${C.faint};margin-left:4px}
.gauge-pct{margin-left:9px;padding:1px 6px;border-radius:9px;font-size: var(--text-2xs);font-weight:600;
  color:color-mix(in srgb,${C.green} 82%,${C.ink});background:color-mix(in srgb,${C.green} 12%,transparent)}
.gaugebar{position:relative;height:12px;border-radius:3px;background:rgba(255,255,255,.05);
  border:1px solid ${C.edge};overflow:hidden}
.gaugebar-fill{position:absolute;left:0;top:0;bottom:0;border-radius:2px;
  background:linear-gradient(90deg,color-mix(in srgb,${C.green} 42%,transparent),${C.green});
  box-shadow:0 0 10px color-mix(in srgb,${C.green} 30%,transparent);transition:width .4s ease}
.gaugebar-edge{position:absolute;right:0;top:0;bottom:0;width:2px;background:${C.green};
  box-shadow:0 0 7px color-mix(in srgb,${C.green} 70%,transparent);animation:pulse 2.6s ease-in-out infinite}
.gaugebar-tick{position:absolute;top:2px;bottom:2px;width:1px;background:rgba(255,255,255,.10)}

/* CADENCE — slim activity meter: how busy */
.cadence{display:flex;align-items:center;gap:10px}
.cadence-meter{display:flex;align-items:flex-end;gap:2px;height:18px;flex:none}
.cad-bar{width:3px;border-radius:1px;background:color-mix(in srgb,${C.green} 26%,transparent)}
.cad-bar--live{background:${C.green};box-shadow:0 0 5px color-mix(in srgb,${C.green} 50%,transparent);animation:pulse 2.6s ease-in-out infinite}
.cadence-read{font-family:ui-monospace,monospace;font-size: var(--text-xs);color:${C.dim}}
.cadence-read b{color:${C.muted};font-weight:600;font-variant-numeric:tabular-nums}
...
/* kv grid (cluster · runtime) */
.kvgrid{display:flex;flex-direction:column;gap:0;padding:0 14px}
.kv{display:grid;grid-template-columns:84px 1fr;column-gap:10px;align-items:center;padding:4px 0;min-width:0}
...
/* raw token dials — secondary readout under the cluster, flat (no boxes) */
.dials{display:grid;grid-template-columns:repeat(3,1fr);gap:9px 12px;padding:9px 14px 2px;margin-top:6px;
  border-top:1px solid ${C.edgeSoft}}
.dial{display:flex;flex-direction:column;gap:2px;min-width:0}
.dial-n{font-family:ui-monospace,monospace;font-size: var(--text-sm);color:${C.muted};font-variant-numeric:tabular-nums;
  white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.dial-l{font-family:ui-monospace,monospace;font-size: var(--text-3xs);color:${C.faint};text-transform:lowercase;letter-spacing:.06em}
