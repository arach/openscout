...
function HeaderChip({
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 truncate font-mono text-sm font-medium text-studio-ink">
        {value}
      </span>
      <ChevronDown
        size={10}
...
export function PrAssignReviewStudy() {
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-mono text-2xs font-semibold uppercase tracking-[0.14em] text-studio-ink-faint">
              Review
            </span>
          ...
          <div className="mt-0.5 truncate font-sans text-lg font-medium text-studio-ink">
            {PR.title}
          </div>
          <div className="mt-0.5 truncate font-mono text-xs text-studio-ink-faint">
            ...
            {(["new", "existing"] as const).map((mode) => {
              ...
              return (
                <button
                  ...
                  className={cx(
                    "focus-ring rounded-full px-2.5 py-1 font-mono text-xs transition-colors",
                    active
      ...
      {openMenu === "project" ? (
        <div className="canvas-tint rounded-lg border border-studio-edge p-1.5">
          <div className="mb-1 px-2 pt-1 font-mono text-2xs uppercase tracking-[0.12em] text-studio-ink-faint">
              ...
              return (
                ...
                >
                  <span className="min-w-0 flex-1">
                    <span className="block truncate font-sans text-md font-medium text-studio-ink">
                      {item.label}
                      <span className="ml-1.5 font-mono text-sm font-normal text-studio-ink-faint">
                        · {item.branch}
                      </span>
                    </span>
                    <span className="block truncate font-mono text-xs text-studio-ink-faint">
                      {shortPath(item.path)}
                    </span>
                  </span>
                  <span
                    className={cx(
                      "shrink-0 rounded-full px-1.5 py-0.5 font-mono text-2xs uppercase tracking-[0.08em]",
      ...
      {openMenu === "agent" ? (
        <div className="canvas-tint rounded-lg border border-studio-edge p-1.5">
          ...
          <div className="max-h-52 space-y-0.5 overflow-y-auto">
            ...
            >
              <span className="grid h-7 w-7 place-items-center rounded-md bg-status-neutral-bg font-mono text-sm text-status-neutral-fg">
                ·
              </span>
              <span className="min-w-0 flex-1">
                <span className="block font-sans text-md font-medium text-studio-ink">
                  One-time reviewer
                </span>
                <span className="block font-mono text-xs text-studio-ink-faint">
                  new session on {project.label}
                </span>
              </span>
              <span className="shrink-0 font-mono text-2xs uppercase tracking-[0.08em] text-status-ok-fg">
                default
              </span>
            </button>
            {rankedAgents.map((item) => (
              <button
                ...
                className="focus-ring ink-row flex w-full items-center gap-2.5 rounded-md px-2.5 py-2 text-left transition-colors"
              >
                <span className="grid h-7 w-7 place-items-center rounded-md bg-status-info-bg font-mono text-sm font-bold text-status-info-fg">
                ...
                <span
                  className={cx(
                    "shrink-0 font-mono text-2xs uppercase tracking-[0.08em]",
  ...
  return (
    <div className="space-y-8">
      {/* Intent strip */}
      <div className="grid gap-3 rounded-md border border-studio-edge bg-studio-surface p-4 font-mono text-sm text-studio-ink-faint sm:grid-cols-3">
        <div>
      ...
      <div className="mx-auto max-w-[560px]">
        ...
        {last ? (
          <p className="mt-2 font-mono text-sm text-studio-ink-faint">
