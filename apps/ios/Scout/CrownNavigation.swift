...
private struct CrownButton: View {
    ...
    /// Pro gesture: long-hold the crown, then slide UP past the threshold to
    /// summon the tail. Sequenced long-press → drag, so plain taps fall through
    ...
    @State private var tailArmed = false
    /// Latch so one hold fires at most once — drag updates keep arriving after
    /// the threshold crossing and would otherwise re-summon.
    @State private var tailSummoned = false

    /// Upward drag distance past which an armed hold summons the tail.
    private static let tailDragThreshold: CGFloat = 40
    ...
    var body: some View {
        ...
        // the button's own tap stays the crown's primary action.
        .gesture(
            LongPressGesture(minimumDuration: 0.35)
                ...
                .onChanged { value in
                    ...
                    case .second(true, let drag):
                        armTailGesture()
                        if let drag, !tailSummoned, -drag.translation.height > Self.tailDragThreshold {
                            tailArmed = false
                            tailSummoned = true
                            #if canImport(UIKit)
    ...
    private func armTailGesture() {
        guard !tailArmed, !tailSummoned else { return }
        tailArmed = true
        #if canImport(UIKit)
...
private struct LEDCore: View {
    /// One paired Mac in LED grammar: a signal dot (lit + glowing when online)
    /// and the host's name. Wide-face only — the phone keeps the bare count.
    struct HostLED: Identifiable, Equatable {
        let id: String
        let name: String
        let online: Bool
    }
    ...
    var body: some View {
        HStack(spacing: compact ? 8 : 11) {
            ...
            // online" reads before agent counts. Wide face: one chip per Mac
            // (dot + name, online first) so the header shows WHO is connected,
            // not just how many. Phone/fallback: the aggregate count, dot lit
            // when any Mac is up.
            if !hosts.isEmpty && !compact {
                HStack(spacing: 10) {
                    ForEach(hosts.prefix(3)) { host in
                        HStack(spacing: 4) {
                            Circle()
                                .fill(host.online ? CrownTheme.signal : CrownTheme.dimLED)
...
private struct MorphLED: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            wingDivider
            LEDCore(agents: agents, active: active, hostsOnline: hostsOnline, hostsTotal: hostsTotal, hosts: hosts)
...
struct CrownNavChrome: View {
    ...
    var onConnect: () -> Void
    var onLED: () -> Void
    /// Pro gesture off the crown (long-hold + slide up) — presents the tail
    /// regardless of the home tail module toggle.
    var onTailSummon: () -> Void = {}
    ...
    // wide canvas Lanes and Deck both earn seats — the strip's pill pair has
    // no third slot, and the compact island row has room for them. They split
    // THREE PER SIDE: an unbalanced row shifts the crown gap off the island's
    // center and the crown (a centered overlay) swallows the last seat — the
    // pass-6 "Deck behind the crown" bug. Deck takes the routed-send mark so
    // the two don't share one glyph.
    private var innerLeft: [(GlyphShape.Kind, String, RootView.Surface)] {
        ...
        if isWide { items.append((.lanes, "Lanes", .lanes)) }
        return items
    }
    private var innerRight: [(GlyphShape.Kind, String, RootView.Surface)] {
        var items: [(GlyphShape.Kind, String, RootView.Surface)] = []
        if isWide { items.append((.dispatch, "Deck", .deck)) }
        items.append((.comms, "Comms", .comms))
        items.append((.terminal, "Term", .terminal))
        return items
    }
    ...
    /// Per-Mac chips for the wide LED face: online first (they're the ones
    /// doing work), then alphabetical so the row doesn't jitter as last-seen
    /// ordering shifts under us.
    private var hostLEDs: [LEDCore.HostLED] {
        model.pairedMachines
            .sorted { ($0.isOnline ? 0 : 1, $0.name.lowercased()) < ($1.isOnline ? 0 : 1, $1.name.lowercased()) }
            .map { LEDCore.HostLED(id: $0.id, name: $0.name, online: $0.isOnline) }
    }

    private func bottomCluster(_ geo: GeometryProxy) -> some View {
        ...
        // One computed gap between EVERY adjacent pair of bottom elements —
        // corners, seats, AND the crown's reserved hole — so crown-to-seat reads
        // identical to seat-to-seat (the old Spacer(minLength: 4) + fixed crown
        // hole pinned the crown to its neighbors while the outer gaps floated).
        // Available width = the cluster's own frame (620 iPad / screen − hPad×2
        // phone). nElements = 2 corners + nSeats + 1 crown hole; gaps = nElements−1.
        let nSeats = innerLeft.count + innerRight.count
        let clusterWidth = isWide ? min(620, geo.size.width) : geo.size.width
        let contentWidth = clusterWidth - hPad * 2
        let equalGap = (contentWidth - sizing.corner * 2 - sizing.seat * CGFloat(nSeats) - sizing.crown)
            / CGFloat(nSeats + 2)
        // Floor at 4pt: a mini phone can't afford the equal gap, so it keeps the
        // legacy mixed spacing (minLength spacers + fixed crown hole) unchanged.
        let useEqualGaps = equalGap >= 4
        return ZStack {
            ...
            HStack(alignment: .center, spacing: 0) {
                ...
                if useEqualGaps {
                    ...
                    Spacer().frame(width: sizing.crown)
            ...
            CrownButton(active: assembled, alive: alive, loading: loading, diameter: sizing.crown, hexSize: sizing.hex, onTailSummon: { poke(); onTailSummon() }) {
                #if canImport(UIKit)
                UIImpactFeedbackGenerator(style: .soft).impactOccurred()
                #endif
                assembled.toggle()
...
struct CrownVitalsPanel: View {
    ...
    private var panelHeight: CGFloat {
        ...
        let usage = model.serviceBudgets.isEmpty ? 0 : 30 + CGFloat(min(model.serviceBudgets.count, 3)) * 58
    ...
    /// One provider's quota, in LED grammar: a bold provider tag + plan over the
    /// same animated window gauges the old Home strip carried (QuotaWindowMeter) —
    /// the richness moved here when Home slimmed to the sparkline. Read-only glance.
        ...
        VStack(alignment: .leading, spacing: 6) {
            HStack(spacing: 10) {
                ...
                if !budget.plan.isEmpty {
                    Text(budget.plan.uppercased())
                        .font(HudFont.mono(8, weight: .medium)).tracking(0.5)
                        .foregroundStyle(ScoutInk.dim)
                        .lineLimit(1)
                }
            }
            VStack(alignment: .leading, spacing: HudSpacing.xs) {
                ForEach(Array(budget.windows.prefix(2).enumerated()), id: \.offset) { _, window in
                    QuotaWindowMeter(window: window, motionEnabled: true)
                }
            }
        }
        .padding(.vertical, 8)
        .accessibilityElement(children: .combine)
        .accessibilityLabel(usageAccessibilityLabel(budget))
    }

    private func usageAccessibilityLabel(_ budget: ServiceBudget) -> String {
        let windows = budget.windows.prefix(2).map { "\($0.label) \(Int($0.usedPercent.rounded())) percent used" }
        return "\(budget.label) quota: " + windows.joined(separator: ", ")
    }
