...
// MARK: - Cockpit canvas
//
// The shared `HudPhoneAppShell` paints a flat near-black. ScoutNext layers its
...
struct ScoutCanvas: View {
    var body: some View {
        ZStack {
            ...
            // Top slightly lifted, a touch cool; floor deepened. Keeps large
            // empty surfaces from looking like dead black.
            LinearGradient(
                stops: [
                    .init(color: Color(red: 0.086, green: 0.094, blue: 0.103), location: 0.0),
                    .init(color: HudPalette.bg, location: 0.40),
                    .init(color: Color(red: 0.043, green: 0.043, blue: 0.051), location: 1.0)
                ],
                startPoint: .top, endPoint: .bottom
            )

            // Emerald key light under the wordmark (top-leading). `.screen` adds
            // light on dark without washing the blacks.
            RadialGradient(
                colors: [HudPalette.accent.opacity(0.14), HudPalette.accent.opacity(0.0)],
