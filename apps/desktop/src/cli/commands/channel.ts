...
export function renderChannelCommandHelp(): string {
  return [
    ...
    "",
    "Scout-managed tmux Claude sessions inject this server automatically.",
    "For manual Claude Code setup (.mcp.json):",
    '  { "mcpServers": { "scout-channel": {',
    '      "command": "scout",',
    '      "args": ["channel", "--context-root", "<project>"]',
    "  } } }",
    "",
    "Channel MCP tools:",
    "  scout_whoami, scout_channels_list, scout_inbox_latest, scout_inbox_pending,",
    "  scout_channel_latest, scout_mark_read, scout_reply, scout_send",
    "",
    "Do not name this server scout if you also use the full Scout MCP server.",
    "The channel server covers mesh read/catch-up/reply; scout mcp exposes",
