...
import { compactHomePath, interAgentProfileKindLabel } from '@web/features/messages/lib/relay-utils';
...
export function useSettingsHelpViewProps({
  ...
  const selectedSettingsAgent = React.useMemo(
    () => (
      settingsAgentId
        ? interAgentAgents.find((agent) => agent.id === settingsAgentId) ?? null
        : null
    ),
    [interAgentAgents, settingsAgentId],
  );
  const selectedSettingsAgentConfigReady = agentController.selectedInterAgent?.id === settingsAgentId;
  const visibleSettingsAgent = selectedSettingsAgentConfigReady
    ? agentController.selectedInterAgent
    : selectedSettingsAgent;

  const handleSetSettingsSection = React.useCallback((section: SettingsSectionId) => {
    setSettingsSection(section);
    setSettingsAgentId(null);
  }, [setSettingsAgentId, setSettingsSection]);
  ...
  const headerActions = settingsSection === 'profile' ? (
    ...
    <>
      ...
      {visibleSettingsAgent && selectedSettingsAgentConfigReady && agentController.hasEditableAgentConfig ? (
        agentController.isAgentConfigEditing ? (
          ...
          <button
            ...
            disabled={!settingsAgentId || agentController.agentConfigLoading || agentController.agentConfigRestarting}
          >
            Edit Agent
          </button>
        )
  ...
  return {
    ...
    settings: {
      ...
      settingsSections,
      activeSettingsMeta,
      onSetSettingsSection: handleSetSettingsSection,
      ...
      agentSettingsViewProps: {
        styles,
        selectedInterAgent: visibleSettingsAgent,
        availableAgents: agentController.rosterInterAgentAgents,
        isAgentConfigEditing: selectedSettingsAgentConfigReady ? agentController.isAgentConfigEditing : false,
        hasEditableAgentConfig: selectedSettingsAgentConfigReady ? agentController.hasEditableAgentConfig : false,
        agentConfigLoading: settingsAgentId ? (!selectedSettingsAgentConfigReady || agentController.agentConfigLoading) : false,
        agentConfigSaving: selectedSettingsAgentConfigReady ? agentController.agentConfigSaving : false,
        agentConfigRestarting: selectedSettingsAgentConfigReady ? agentController.agentConfigRestarting : false,
        visibleAgentConfig: selectedSettingsAgentConfigReady ? agentController.visibleAgentConfig : null,
        agentConfigFeedback: selectedSettingsAgentConfigReady ? agentController.agentConfigFeedback : null,
        agentCapabilitiesPreview: selectedSettingsAgentConfigReady ? agentController.agentCapabilitiesPreview : [],
        ...
        onSelectAgent: (agentId) => {
          ...
          agentController.handleSelectInterAgent(agentId);
        },
      },
