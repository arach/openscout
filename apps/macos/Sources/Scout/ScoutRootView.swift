...
struct ScoutRootView: View {
    ...
    private func moveSelection(_ delta: Int) {
        ...
        case .repos:
            reposTreeMove(delta)
        case .tail, .lanes, .dispatch, .settings:
            break
        }
    }
    ...
    private func chromeTitlebarActions(layout: ScoutShellLayout) -> [HudChromeTitlebarAction] {
        ...
        if section != .settings && section != .dispatch {
    ...
    private func trailingPanel(layout: ScoutShellLayout) -> some View {
        Group {
            if section == .settings || section == .dispatch {
                // Dispatch is a full-bleed broker ledger embed that carries its
                // own inline inspector, so it skips the native trailing panel.
                EmptyView()
    ...
    var body: some View {
        HStack(alignment: .top, spacing: HudSpacing.xl) {
            ScoutTimelineAvatarView(
                name: "Operator",
                size: 28,
                ring: ScoutPalette.accent.opacity(0.5)
            )
...
private struct ScoutPendingConversationProgressRow: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: HudSpacing.xl) {
            ScoutTimelineAvatarView(name: actorName, size: 28)
