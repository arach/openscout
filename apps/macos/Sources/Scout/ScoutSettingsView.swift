...
struct ScoutSettingsView: View {
    ...
    private var terminalPage: some View {
        VStack(alignment: .leading, spacing: HudSpacing.xxxl) {
            ...
            settingsBlock(title: "External terminal") {
                VStack(alignment: .leading, spacing: HudSpacing.md) {
                    Text("Used by “Open in system terminal” / “Open in Herdr” with a prefilled attach command.")
                        .font(HudFont.ui(HudTextSize.xs))
                        .foregroundStyle(ScoutPalette.muted)

                    settingRow(title: "Preferred app") {
                        Picker("Preferred terminal", selection: $preferredTerminalAppPath) {
                            Text("Automatic").tag("")
                            ForEach(preferredTerminalCandidates, id: \.path) { candidate in
    ...
    private var preferredTerminalCandidates: [(name: String, path: String)] {
        let paths = [
        ...
        return paths.compactMap { name, path in
            guard FileManager.default.fileExists(atPath: path), !seen.contains(path) else { return nil }
            seen.insert(path)
            return (name, path)
        }
    }
