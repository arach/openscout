...
/**
 * OpenCode reports a provider-side rejection — an unentitled model, an expired
 * key, a workspace with no payment method — as a *successful* turn that emitted
 * no text and burned no tokens. Left alone that reaches Scout as an agent which
 * replied with nothing, which is far worse than a failure: it looks like a
 * working provider and silently poisons a rotation. A real turn always spends
 * input tokens on the prompt, so zero usage plus no output is never legitimate.
 */
function assertProviderProducedATurn(
  ...
  throw new Error(
    ...
    + ` OpenCode reports provider rejections this way, so the model is most likely`
    + ` unavailable to this account — paid OpenCode Zen models need a payment method`
    + ` on the workspace. Run \`opencode run -m <model> "hi"\` to see the underlying error.`,
  );
}
