...
describe("agents route parsing", () => {
  ...
  test("agent chat routes preserve opaque chat ids", () => {
    const route = routeFromUrl("http://127.0.0.1:3200/agents/openscout-6.main.mini/c/c.openscout-chat");
  ...
  test("machine scope composes with existing route query params", () => {
    ...
    const conversationRoute = routeFromUrl("http://127.0.0.1:3200/c/c.hudson-chat?compose=ask&machineId=node-b");
    ...
    const messagesRoute = routeFromUrl(
      "http://127.0.0.1:3200/messages/c.font-studio?filter=channel&sort=unread&machineId=node-b",
    );
  ...
  test("observe deep links preserve the explicit observe tab", () => {
    const route = routeFromUrl("http://127.0.0.1:3200/agents/openscout-6.main.mini/c/c.openscout-chat?tab=observe");
  ...
  test("follow routes preserve Scout ids and preferred view", () => {
    const route = routeFromUrl(
      "http://127.0.0.1:3200/follow?view=tail&flightId=flight-1&invocationId=inv-1&conversationId=c.hudson-chat&workId=work-1&targetAgentId=hudson.main",
    );
  ...
  test("messages route preserves conversationId, filter, and sort", () => {
    const route = routeFromUrl(
      "http://127.0.0.1:3200/messages/c.font-studio?filter=channel&sort=unread",
    );
    expect(route).toEqual({
      view: "messages",
      conversationId: "c.font-studio",
