/**
 * shadcn sidebar component family (Base UI variant) — SCO-084.
 *
 * Upstream source:
 *   shadcn-ui/ui apps/v4/registry/bases/base/ui/sidebar.tsx
 *   commit f31ed81983653919dd4fe77aee4b4859f610f1dc
 * Resolved registry style: base-nova (full Tailwind classes, not cn-* placeholders).
 *
 * Port notes (Requirement 0 / 2):
 * - @base-ui/react/* imports → @base-ui-components/react@1.0.0-rc.0 (installed).
 *   Do NOT add the renamed @base-ui/react package alongside.
 * - Cookie write for sidebar_state deleted (usePersistentState owns persistence).
 * - Cmd/Ctrl+B provider listener deleted (shell owns Cmd+B with editable/terminal guard).
 * - Mobile / offcanvas path deleted entirely: openMobile, isMobile, use-mobile,
...
/** Expanded rail width — matches SIDEBAR_EXPANDED_WIDTH / --sidebar-width. */
const SIDEBAR_WIDTH = "260px";
/** Icon rail width — matches SIDEBAR_COLLAPSED_WIDTH / --sidebar-width-icon. */
...
function SidebarProvider({
  // Icon-rail-first default (SCO-084); shell drives controlled open anyway.
  defaultOpen = false,
  open: openProp,
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // No cookie write — shell useSidebarCollapse / usePersistentState owns persistence.
    },
    [setOpenProp, open],
  );
  ...
  // Provider keyboard shortcut intentionally removed (Requirement 6): shell owns Cmd+B.
    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delay={200}>
        <div
          ...
          // `contents` — no layout box, no hit target. Never pointer-events-auto here.
...
function Sidebar({
  ...
  /** "icon" (default) or "none". Offcanvas/mobile deleted (Requirement 2). */
  ...
  if (collapsible === "none") {
    return (
      <div
        ...
        className={cn(
          "pointer-events-auto flex h-full w-(--sidebar-width) flex-col bg-sidebar text-sidebar-foreground",
          className,
        )}
        {...props}
      >
  ...
  // No mobile Sheet branch. Fixed HUD geometry only.
  return (
    <div
    ...
    >
      {/*
        Container only: pointer-events-auto here (not on provider).
        top: below nav (48px); bottom: StatusBar 28px. See file header.
      */}
      <div
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      title={state === "collapsed" ? "Expand sidebar (⌘B)" : "Collapse sidebar (⌘B)"}
...
function SidebarInput({
  ...
  return (
    <Input
      ...
      data-sidebar="input"
      className={cn("h-8 w-full bg-transparent shadow-none", className)}
      {...props}
    />
  );
}
