...
export interface MobileTerminalProvisionResult {
  ...
  port: number;
  username: string;
  /** SHA256:… of the Mac's host key when pinned; null ⇒ phone uses trust-on-first-use. */
...
export function provisionMobileTerminalAccess(
  ...
  if (result.changed) {
    ...
    writeFileSync(authorizedKeysPath, result.authorizedKeys, { mode: 0o600 });
    try {
  ...
  return {
    ...
    hostKeyFingerprint: readHostKeyFingerprint(),
  };
}
 ...
 * raw key blob). Returns null if the public key can't be read, in which case the
 * phone falls back to trust-on-first-use. ed25519 because that's what NIOSSH
 * negotiates against modern macOS sshd.
 */
function readHostKeyFingerprint(): string | null {
  try {
