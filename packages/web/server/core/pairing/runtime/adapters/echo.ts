...
import { BaseAdapter, type AdapterConfig } from "../protocol/adapter.ts";
import type { Prompt, Turn, Block } from "../protocol/primitives.ts";
...
export class EchoAdapter extends BaseAdapter {
  ...
  decide(blockId: string, decision: "approve" | "deny", reason?: string): void {
    const pending = this.pendingApprovals.get(blockId);
    if (!pending) return;
    pending.reason = reason;
    pending.resolve(decision);
    this.pendingApprovals.delete(blockId);
  }
