...
function briefSystemPrompt(basePrompt: string, mode: ScoutbotBriefMode): string {
  ...
  if (mode === "fleet-home") {
    shared.push(
      ...
      "Priority order: (1) needs-you-now items: approvals, decisions, questions, failed checks, blocked work; (2) open or hidden obligations: asks without replies, sessions idle after an error, repeated failures, ambiguous ownership; (3) material progress: ships, completed work, docs/code changes, verification results; (4) current work only when it has a deliverable, owner, or risk; (5) next best inspection point.",
      "If anything is waiting, blocked, failed, open without a reply, needs human input, or looks risky, make that the first sentence and include owner plus next move when evidence supports it.",
      "Requester-side wait timeouts are UI-side non-events. Do not use the words timeout, timed out, synchronous acknowledgment, stopped waiting, stale, stuck, stalled, stalling, delay, delayed, failed, or stopped for that condition; instead describe the task plane (open/replied) and execution plane (working/no recent agent event) when those facts matter.",
      ...
      "Bad pattern: requester wait-timeout sentence. Good pattern: Open obligation: thread/session/question has not moved, why it matters, where to inspect.",
...
function briefOperatorRequest(ttlMs: number, mode: ScoutbotBriefMode): string {
  ...
  if (mode === "fleet-home") {
    return [
      ...
      "Focus on things requiring attention, subtle signals that could fall through the cracks, open/hidden obligations, material progress with consequence, and useful next inspection points.",
