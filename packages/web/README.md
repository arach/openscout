...
Realtime Scoutbot voice is a flagged high-trust pilot. The client flag only exposes the capability; the billable server route stays closed unless the host starts the web server with `OPENSCOUT_REALTIME_VOICE_ENABLED=1`. The operator must opt in under **Settings → Voice**, then explicitly starts each call from the footer Voice control. Calls use the configured server-side OpenAI API key, and Scoutbot requires a separate confirmation before sending any proposed agent request. Allowlisted, non-destructive app navigation can be applied directly during a call. Host-local SQLite leases default to one active call and four starts per minute; see [`docs/artifacts/realtime-voice-design-pass.md`](../../docs/artifacts/realtime-voice-design-pass.md) for the tuning variables and boundary details.
