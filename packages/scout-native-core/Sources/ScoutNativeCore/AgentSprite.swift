...
/// Deterministic generative agent sprite — the shared native sibling of the web
...
///
/// The hash + PRNG + cell layout are **bit-exact** with the TypeScript version
/// (see `AgentSpriteTests`, whose vectors are generated from the JS engine), so
/// the same name yields the same creature on web, iOS and macOS.
///
/// The mapping the system encodes:
///   - shape      ← the name    (the silhouette + eyes + traits)
///   - hue        ← the name    (curated wheel), or forced by the caller
///   - brightness ← the state   (`AgentSpriteTone`)
///
/// Pure logic only (no SwiftUI) so any target can use it; rendering lives with
/// each app (iOS: `SpriteAvatarView.swift`).
///
/// NOTE: `apps/macos/Sources/ScoutAppCore/AgentSprite.swift` is the older
/// sibling of this file — same algorithm, macOS-only, and coupled to
/// `ScoutAgentState` for its tone mapping. macOS already depends on
/// ScoutNativeCore, so it should retire onto this copy; until it does, any
/// change here must be mirrored there (the test vectors below are the tripwire).

public enum AgentSpriteCell: Sendable, Equatable {
...
public struct AgentSpriteTone: Sendable, Equatable {
    ...
    /// Alive — working, or asking you something.
    public static let live = AgentSpriteTone(l: 0.75, c: 0.16)
    /// Reachable but not running.
    public static let idle = AgentSpriteTone(l: 0.73, c: 0.13)
    /// Finished; the creature cools off.
    public static let done = AgentSpriteTone(l: 0.64, c: 0.075)
    /// Gone — nearly grey.
    public static let offline = AgentSpriteTone(l: 0.5, c: 0.02)
}
...
public enum AgentSpriteFactory {
    ...
    /// Hue per harness family, for callers that want the creature tinted by
    /// runtime instead of by name. Unknown harnesses return nil so the caller
