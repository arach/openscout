// State tracker — accumulates session state from streaming Dispatch events.
//
// The bridge feeds every DispatchEvent into the tracker.  At any point, a client
...
export class StateTracker {
  ...
  /** Process one Dispatch event and update internal state. */
  trackEvent(sessionId: string, event: DispatchEvent): void {
  ...
  private handleTurnStart(
    state: SessionState,
    event: Extract<DispatchEvent, { event: "turn:start" }>,
  ): void {
  ...
  private handleTurnEnd(
    state: SessionState,
    event: Extract<DispatchEvent, { event: "turn:end" }>,
  ): void {
  ...
  private handleTurnError(
    state: SessionState,
    event: Extract<DispatchEvent, { event: "turn:error" }>,
  ): void {
    const turn = this.findTurn(state, event.turnId);
    if (!turn) return;
  ...
  private handleBlockStart(
    state: SessionState,
    event: Extract<DispatchEvent, { event: "block:start" }>,
  ): void {
    const turn = this.findTurn(state, event.turnId);
    if (!turn) return;
  ...
  private handleBlockTextDelta(
    state: SessionState,
    event: Extract<DispatchEvent, { event: "block:delta" }>,
  ): void {
  ...
  private handleBlockActionOutput(
    state: SessionState,
    event: Extract<DispatchEvent, { event: "block:action:output" }>,
  ): void {
    const blockState = this.findBlock(state, event.turnId, event.blockId);
    if (!blockState) return;
  ...
  private handleBlockActionStatus(
    state: SessionState,
    event: Extract<DispatchEvent, { event: "block:action:status" }>,
  ): void {
    const blockState = this.findBlock(state, event.turnId, event.blockId);
    if (!blockState) return;
  ...
  private handleBlockActionApproval(
    state: SessionState,
    event: Extract<DispatchEvent, { event: "block:action:approval" }>,
  ): void {
    const blockState = this.findBlock(state, event.turnId, event.blockId);
    if (!blockState) return;
  ...
  private handleBlockEnd(
    state: SessionState,
    event: Extract<DispatchEvent, { event: "block:end" }>,
  ): void {
    const blockState = this.findBlock(state, event.turnId, event.blockId);
    if (!blockState) return;
