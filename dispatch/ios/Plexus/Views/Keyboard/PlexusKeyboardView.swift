// PlexusKeyboardView — SwiftUI wrapper around PlexusCompactKeyboard (UIKit).
//
...
// MARK: - PlexusKeyboardView (SwiftUI API)

struct PlexusKeyboardView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            if isMinimized {
                ...
                .frame(height: PlexusCompactKeyboard.preferredHeight)
            }
        }
        .background {
            Color.clear
...
private struct PlexusKeyboardRepresentable: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> PlexusKeyboardHostView {
        let host = PlexusKeyboardHostView()
        bindCallbacks(host)
        return host
    }

    func updateUIView(_ host: PlexusKeyboardHostView, context: Context) {
        bindCallbacks(host)
    }

    private func bindCallbacks(_ host: PlexusKeyboardHostView) {
    ...
    private func reportPage(_ host: PlexusKeyboardHostView) {
        let label: String
...
/// Wraps PlexusCompactKeyboard with swipe gesture handling
/// and edge gesture exclusion zones.
final class PlexusKeyboardHostView: UIView, UIGestureRecognizerDelegate {
    let keyboard = PlexusCompactKeyboard()
...
private struct ToolbarButton: View {
    let icon: String
    let label: String?
    var tint: Color = PlexusColors.textSecondary
    let action: () -> Void
