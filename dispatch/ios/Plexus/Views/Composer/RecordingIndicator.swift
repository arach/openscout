...
struct RecordingIndicator: View {
    ...
    private var borderColor: Color {
        switch phase {
        case .recording: PlexusColors.statusError.opacity(0.3)
        case .transcribing: PlexusColors.statusStreaming.opacity(0.3)
        }
    }
    ...
    private var transcribingContent: some View {
        HStack(spacing: PlexusSpacing.sm) {
            ProgressView()
                .controlSize(.small)
                .tint(PlexusColors.statusStreaming)
...
private struct PulsingDot: View {
    ...
    var body: some View {
        Circle()
            ...
            .shadow(color: PlexusColors.statusError.opacity(0.6), radius: pulsing ? 6 : 2)
        ...
        HStack(spacing: barSpacing) {
            ForEach(0..<barCount, id: \.self) { index in
                ...
                RoundedRectangle(cornerRadius: 1.5)
                    .fill(PlexusColors.statusError.opacity(0.6 + Double(level) * 0.4))
                    .frame(
                        width: barWidth,
...
private struct WaveformBar: View {
    ...
    var body: some View {
        RoundedRectangle(cornerRadius: 1.5)
            .fill(PlexusColors.statusError.opacity(0.7))
