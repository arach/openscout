// PlexusVoice — Voice capture and transcription for the Plexus composer.
//
// Two modes:
...
private let log = PlexusLog.voice
...
final class ParakeetModelManager: ObservableObject {
    ...
    @Published var state: PlexusVoice.ModelState = .notDownloaded
    ...
    func transcribe(url: URL) async throws -> String {
        guard let manager = asrManager, state == .ready else {
            throw PlexusVoice.VoiceError.notReady
        }
...
@MainActor
final class PlexusVoice: ObservableObject {
    ...
    private var tempRecordingURL: URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("plexus_recording_\(UUID().uuidString).wav")
    }
    ...
    func prepare() async {
        ...
        // Parakeet model loading is kicked off at app launch (PlexusApp.swift).
        state = .ready
        log.info("PlexusVoice ready (recording + Apple Speech available)")
    }
