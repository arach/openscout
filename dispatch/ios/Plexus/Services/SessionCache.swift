...
final class SessionCache: Sendable {
    ...
    func save(_ state: SessionState) {
        ...
        do {
            ...
            PlexusLog.session.debug("Cached session \(state.session.id): \(trimmed.turns.count) turns")
        } catch {
            PlexusLog.session.warning("Failed to cache session \(state.session.id): \(error.localizedDescription)")
        }
